...
//		if(!interval_pair_overlaps(new_cluster->break_points,scl_tmp)){continue;}
...
void sv_cluster_graph_fix(sv_cluster *c, graph_t *g){
	int i;
	...
	while(graph_iter_has_next(it)){
		...
		if(interval_pair_overlaps(scl,c->break_points)){
