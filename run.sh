...
echo "OPTIONAL: filtering out low coverage clones"
for pool in $(seq 1 $count)
do
	# bamToBed
	bedtools bamtobed -i $bamfile$pool$suffix > $outputDir/temp/pool$pool.bed
...
# find inversions with the given files
